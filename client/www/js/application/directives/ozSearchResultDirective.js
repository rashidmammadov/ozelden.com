(function(){
    ...
    function ozSearchResult($filter, $timeout) {
        ...
        function link(scope, elem, attrs){
            ...
            function drawList(data){
                if(data) {
                    var userCard = d3.selectAll('oz-search-result md-card').data(data);
                    ...
                    userCard.select('.oz-user-card-info .location').html(function(d){d.location;});
                    ...
                    userCard.selectAll('.oz-user-card-action button')._groups.forEach(function(button, cardIndex){
                        button.forEach(function(tab){
                            angular.element(tab).on('click', function(){
                                var pageIndex = angular.element(tab).attr('tab-index');
                                changeTab(cardIndex, pageIndex);
                            });
                        });
                    });
                }
            ...
            function changeTab(cardIndex, index) {
                var userCard = d3.selectAll('oz-search-result md-card');
                var tabs = angular.element(userCard.selectAll('.oz-user-card-content .tab')._groups[cardIndex]);
                var actions = angular.element(userCard.selectAll('.oz-user-card-action button')._groups[cardIndex]);
